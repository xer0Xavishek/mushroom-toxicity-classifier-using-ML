...
Preprocessing: Three-tier approach: 
- (a) Drop 4 columns with >80% missing fabricating 90% of a column defeats the purpose. 
- (b) Mode imputation for 5 columns with 4–62% missing  mode is the only valid central tendency for categorical data.
- (c) StandardScaler required for KNN (distances), Logistic Regression (gradient descent stability), and MLP (activation functions).
